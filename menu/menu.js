      ...
      action: function () {
        ...
        `,
          ...
          .then(function (response) {
            if (!response || !response.success) {
              throw new Error(
                response
                  ? response.error
                  : "No se recibió respuesta del proceso de renderizado",
              );
            }
            ...
            // Paso 2: Parsear el XML para extraer y normalizar los dataFields
            var parsedData = xmlParser.parseDataFieldsFromXML(xml);
            var parsedInfo;
            if (typeof parsedData === "string") {
              parsedInfo = `normalized JSON, ${parsedData.length} caracteres`;
            } else if (Array.isArray(parsedData)) {
              parsedInfo = `array con ${parsedData.length} elementos`;
            } else {
              parsedInfo = JSON.stringify(parsedData).substring(0, 100);
            }
            logger.log(`Datos parseados: ${parsedInfo}`);
            ...
            if (electronApp.mainWindow && electronApp.mainWindow.webContents) {
              htmlDialog
                ...
                .catch(function (err) {
                  ...
                  logger.log(
                    `Error mostrando diálogo de progreso: ${err.message}`,
                    "WARN",
                  );
                });
            }

            // Paso 3: Enviar los datos parseados al servicio de IA (LLM) para generar PlantUML
          ...
          .then(function (data) {
            ...
            if (electronApp.mainWindow && electronApp.mainWindow.webContents) {
              htmlDialog
                ...
                .then(function (buttonIndex) {
                  ...
                  if (buttonIndex === 0) {
                    ...
                    htmlDialog
                      ...
                      .catch(function (err) {
                        logger.log(
                          `Error mostrando diálogo de confirmación: ${err.message}`,
                          "ERROR",
                        );
                      });
                  }
                  // Si el usuario presionó "Ver Logs" (índice 1) y hay logs
                    ...
                    htmlDialog
                      ...
                      .catch(function (err) {
                        logger.log(
                          `Error mostrando diálogo de logs: ${err.message}`,
                          "ERROR",
                        );
                      });
                  }
                ...
                .catch(function (err) {
                  logger.log(
                    `Error mostrando diálogo HTML: ${err.message}`,
                    "ERROR",
                  );
                });
            }
          ...
          .catch(function (error) {
            ...
            logger.log(
              `Archivo de log disponible en: ${logger.getLogPath()}`,
              "INFO",
            );
            ...
            if (electronApp.mainWindow && electronApp.mainWindow.webContents) {
              htmlDialog
                  ...
                  if (buttonIndex === 1) {
                    ...
                    try {
                      ...
                      if (fs.existsSync(logPath)) {
                        ...
                        logContent =
                          recentLogs || "El archivo de log está vacío.";
                      }
                      ...
                      htmlDialog
                        ...
                        .catch(function (dialogErr) {
                          logger.log(
                            `Error mostrando diálogo de log: ${dialogErr.message}`,
                            "ERROR",
                          );
                        });
                    } catch (logErr) {
                      logger.log(
                        `Error leyendo log: ${logErr.message}`,
                        "ERROR",
                      );
                      htmlDialog
                        .showErrorDialog(
                ...
                .catch(function (err) {
                  logger.log(
                    `Error mostrando diálogo de error: ${err.message}`,
                    "ERROR",
                  );
                });
            }
...
function selectOutputDirectory(plantUMLCode, electronApp) {
  ...
  const focusedWindow =
    BrowserWindow.getFocusedWindow() || electronApp.mainWindow;
    ...
    .catch(function (err) {
      ...
      if (electronApp.mainWindow && electronApp.mainWindow.webContents) {
        htmlDialog
          ...
          .catch(function (dialogErr) {
            logger.log(
              `Error mostrando diálogo de error: ${dialogErr.message}`,
              "ERROR",
            );
          });
      }
  ...
  nestjsGenerator
    ...
    .then(function (result) {
      ...
      const successMessage =
        `¡Proyecto NestJS generado exitosamente!\n\n` +
      ...
      htmlDialog
        ...
        .catch(function (err) {
          logger.log(
            `Error mostrando diálogo de éxito: ${err.message}`,
            "ERROR",
          );
        });
    })
